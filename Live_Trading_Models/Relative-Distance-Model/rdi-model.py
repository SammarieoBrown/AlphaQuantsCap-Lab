...
def strategy(symbol, timeframe, sma_period, upper_rdi, lower_rdi):
    ...
    # Buy Logic
    if  (slowEma.iloc[-1]<fastEma.iloc[-1]<df.close.iloc[-1]) and(rdi<lower_rdi):
        signal = 'buy'
        return signal

    # Sell Logic
    elif (slowEma.iloc[-1]>fastEma.iloc[-1]>df.close.iloc[-1]) and (rdi>upper_rdi):
        signal = 'sell'
        return signal
    else:
        signal = 'None'
...
if __name__ == '__main__':
    ...
    assets = {
        ...
        "EURAUD": ["EURAUD",
                   ...
                   80,      # (upper) relative distance index value
                   -80,     # (lower) relative distance index value
                   ],
        "Gold": ["XAUUSD",
                 1.10,
                 mt5.TIMEFRAME_M30,
